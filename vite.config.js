...
// ---------------------------------------------------------------------------
// Gemini Live voice proxy constants.
//
// Gemini Live is the free-tier alternative voice backend: the browser talks to
// THIS dev server over a same-origin WebSocket (/api/gemini/live), and the
// server relays frames to Google's BidiGenerateContent stream so the
// GEMINI_API_KEY never reaches the client. Input is PCM 16 kHz, output PCM
// 24 kHz — both handled on the client (src/voice/geminiLive.js).
// ---------------------------------------------------------------------------
...
// Verify at release: Gemini Live model ids drift. Override via GEMINI_LIVE_MODEL.
const GEMINI_LIVE_MODEL_DEFAULT = 'models/gemini-2.5-flash-native-audio-preview-12-2025';
// Prebuilt Gemini Live voices: Puck, Charon, Kore, Fenrir, Aoede.
const GEMINI_LIVE_VOICE_DEFAULT = 'Kore';
const GEMINI_LIVE_INPUT_SAMPLE_RATE = 16000;
const GEMINI_LIVE_OUTPUT_SAMPLE_RATE = 24000;
// Hard cap on a single relayed WebSocket frame (both directions).
const GEMINI_LIVE_MAX_FRAME_BYTES = 2 * 1024 * 1024;
// Give the upstream socket this long to open before tearing the relay down.
...
/**
 * Gemini Live voice proxy — the free-tier alternative to OpenAI Realtime.
 *
 * Unlike OpenAI (which mints a short-lived session token server-side), Gemini
 * Live needs the raw API key on the WebSocket handshake. The key is therefore
 * brokered through a same-origin WebSocket relay:
 *
 *   browser ──ws──> /api/gemini/live ──wss──> Google BidiGenerateContent?key=…
 *
 * Frames are relayed verbatim in both directions (each side only ever sees
 * JSON). A per-frame byte cap and a connect timeout keep the relay from being
 * used as an open proxy, and the key never reaches the client.
 *
 * `GET /api/gemini/live/config` supplies the session shape the client needs to
 * build its `setup` message — model, voice, instructions, and the shared tool
 * registry — so the voice brain stays a single source of truth in this file.
 */
function relayGeminiLiveFrame(client, upstream, { clientClosed, upstreamClosed }) {
  return {
    clientToUpstream(data, isBinary) {
      if (upstreamClosed()) return;
      if (!frameWithinCap(data)) {
        console.warn('[gemini-live-proxy] oversized client frame; closing relay');
        closeGeminiRelay(client, upstream, { clientClosed, upstreamClosed });
        return;
      }
      upstream.send(data, { binary: Boolean(isBinary) });
    },
    upstreamToClient(data, isBinary) {
      if (clientClosed()) return;
      if (!frameWithinCap(data)) {
        console.warn('[gemini-live-proxy] oversized upstream frame; closing relay');
        closeGeminiRelay(client, upstream, { clientClosed, upstreamClosed });
        return;
      }
...
function frameWithinCap(data) {
  if (typeof data === 'string') return Buffer.byteLength(data) <= GEMINI_LIVE_MAX_FRAME_BYTES;
  if (Buffer.isBuffer(data)) return data.length <= GEMINI_LIVE_MAX_FRAME_BYTES;
  return true;
}

function closeGeminiRelay(client, upstream, { clientClosed, upstreamClosed }) {
  if (!clientClosed()) {
    try { client.close(); } catch { /* no-op */ }
  }
  if (!upstreamClosed()) {
    try { upstream.terminate(); } catch { /* no-op */ }
  }
}

function geminiLiveRelay(WebSocketCtor, client, apiKey) {
  const upstreamUrl = `${GEMINI_LIVE_WS_URL}?key=${encodeURIComponent(apiKey)}`;
  const upstream = new WebSocketCtor(upstreamUrl);
  const state = { clientClosed: false, upstreamClosed: false, opened: false };

  const clientClosed = () => state.clientClosed;
  const upstreamClosed = () => state.upstreamClosed;
  state.clientClosed = false;
  state.upstreamClosed = false;

  const relay = relayGeminiLiveFrame(client, upstream, { clientClosed, upstreamClosed });

  const connectTimer = setTimeout(() => {
    if (!state.opened) {
      console.warn('[gemini-live-proxy] upstream connect timed out');
      closeGeminiRelay(client, upstream, { clientClosed, upstreamClosed });
    }
  }, GEMINI_LIVE_CONNECT_TIMEOUT_MS);
  connectTimer.unref?.();

  client.on('message', (data, isBinary) => relay.clientToUpstream(data, isBinary));
  upstream.on('open', () => { state.opened = true; });
  upstream.on('message', (data, isBinary) => relay.upstreamToClient(data, isBinary));
  upstream.on('error', (error) => {
    console.warn('[gemini-live-proxy] upstream error:', error?.message || error);
    closeGeminiRelay(client, upstream, { clientClosed, upstreamClosed });
  });
  upstream.on('close', () => { state.upstreamClosed = true; try { client.close(); } catch { /* no-op */ } });
  client.on('error', () => {
...
function geminiLiveProxy() {
  const installConfig = (middlewares) => {
    middlewares.use('/api/gemini/live/config', (req, res) => {
      if (req.method !== 'GET') {
        res.statusCode = 405;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ error: 'Method not allowed' }));
  ...
  const installWs = (httpServer) => {
    ...
    httpServer.on('upgrade', (req, socket, head) => {
      ...
      if (pathname !== '/api/gemini/live') return;
      ...
      let wsImpl;
      try {
        wsImpl = createRequire(import.meta.url)('ws');
      } catch (error) {
        console.warn('[gemini-live-proxy] `ws` unavailable:', error?.message || error);
        socket.write('HTTP/1.1 500 Internal Server Error\r\nConnection: close\r\n\r\n');
        socket.destroy();
        return;
      }

      const { WebSocketServer, WebSocket: UpstreamSocket } = wsImpl;
      const wss = new WebSocketServer({ noServer: true });
      wss.handleUpgrade(req, socket, head, (client) => {
        geminiLiveRelay(UpstreamSocket, client, apiKey);
      });
    });
  };
  ...
  return {
    ...
    configureServer(server) {
      installConfig(server.middlewares);
      installWs(server.httpServer);
    },
    configurePreviewServer(server) {
      installConfig(server.middlewares);
      installWs(server.httpServer);
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      //
      // GEV_ALLOW_FRAMING=1 is an explicit, opt-in override for hosted preview
      // sandboxes (e.g. Arena/E2B) that serve the app inside an iframe. It is
      // OFF by default — a normal local/LAN launch keeps the DENY headers.
      headers: env.GEV_ALLOW_FRAMING === '1'
        ? {}
        : {
    ...
    define: {
      ...
      // Voice provider selection is server-authoritative: the client picks the
      // Gemini Live backend only when a GEMINI_API_KEY is configured, otherwise
      // it keeps the existing OpenAI Realtime path. No key value is exposed —
      // just the chosen provider.
      'import.meta.env.GEV_VOICE_PROVIDER': JSON.stringify(env.GEMINI_API_KEY ? 'gemini' : 'openai'),
    },
    build: {
